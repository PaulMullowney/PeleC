...
# Do not load amdgpu-libdrm here: it can break HIP managed allocations (e.g. hipMallocManaged) for this case.
...
LAUNCHER="${LAUNCHER:-mpirun -np $RANKS --mca coll_ucc_enable 1 --mca coll_ucc_priority 100 --mca pml ucx --mca osc ucx --mca opal_rocm_support 1 -x HIP_MEM_POOL_SUPPORT=1 -x HSA_ENABLE_SDMA=0  -x UCX_TLS=sm,self,rocm --map-by ppr:1:numa:PE=16 --bind-to core $AFFINITY_SH}"
